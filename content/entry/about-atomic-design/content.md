...
[Atomic Design](https://atomicdesign.bradfrost.com/) の記事を読み、実務で適用し、ぶつかった課題に対する考察を作成している.

後日、自分の知識への棚卸しや振り返りを行う際の参考資料としても用いる予定.
...
[Atomic Design](https://atomicdesign.bradfrost.com/) から構想を受け実践し落とし込んだ考えを以下の一言で表現した.

「プロダクトの知識を保有する量に基づいて粒度が変わっていく設計手法である.」

## Atomic Design は何を解決するためのものか

「Atomic Design」は、紙をモチーフとした Web ページの設計からスタートし、ディスプレイのサイズに従ったデザインを組む必要があったため生まれてきた考え方だと捉えた.

肥大化していく文字以外の表現方法の要望、増加するディスプレイの種類に応えるため、単一のページとしてではなく、部品の単位で UI を構築していく必要があった.

単一のページを部品として切り刻んでいく中、部品間でもデザインや共通認識としての一貫性が必要になった.

それらを分類、整理するための仕組みとしての Design System が必要になった.

分類、整理するための方法として導出された、階層化を主とした管理方法が「Atomic Design」に繋がっていったと私は捉えた.
...
ページとしてのイメージにそぐわないようになりその受け皿のメタファーとして「レゴブロック」が使われるようになった.

**> 3. 肥大化する要望に対して迅速に応える必要があった.**

肥大化する要望に対して正気を保ちながら対処するため、大きい責任をより小さく分割していくことを避けることができなくなった.
分割の対応として、モジュール化、スタイルガイドの定義、デザインパターン化というアプローチ方法が取られた.
...
プロダクトの知識が含むにつれて抽象的なモジュールから具体的なモジュールへと具現化している.
一番左の区分について考察を深めていく.
...
しかし、それはボタンやフォーム自身は知ることなく、右へ区分が進むにつれてどのプロダクトの場所を表現する要素かの役割を決定するようになる.

例: React Native Elements

> https://reactnativeelements.com/docs/button

私見としては、この区分は、取り掛かりやすくまた上記で示した[例](https://reactnativeelements.com)のようにUIのライブラリとして切り出していたりして流用もしやすい.

実務上では、上記のUIライブラリを用いつつ不必要なプロパティを閉じるようにしてラップしたモジュールで運用していた.
イベント処理は、中で定義/分岐処理をせず引数で渡すようにした.
...
私見として、この区分と次の区分の線引きが曖昧になっていることがあった.
どこの画面でも流用できる塊は、この区分に加えることがあった.
